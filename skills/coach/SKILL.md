...
**Availability:** call `orient` first — its `pending_tools` lists the `gather` steps still being built (discovery/lead-gen is next, GitHub ingestion after). When the catalog or portfolio is empty, coach from the resume and general market knowledge; never invent demand or projects.
